...
// var stylez = [{"stylers":[{"hue":"#ff1a00"},{"invert_lightness":true},{"saturation":-100},{"lightness":33},{"gamma":0.5}]},{"featureType":"water","elementType":"geometry","stylers":[{"color":"#2D333C"}]}];
    ...
    streetViewControl: true //my favorite feature in V3!
     ...
     //   bikeLayer.setMap(map);
        ...
        ags_light_rail = new gvector.AGS({
                    url: "http://arcgis.dvrpc.org/arcgis/rest/services/Transportation/CircuitTrails/MapServer/0",
                    fields: "*",
                    scaleRange: [6, 20],
                    showAll: false,
        ...
        {
            value: "Existing", // If feature.properties.DISTRICT == "A"
  ...
  //  infoWindowTemplate: '<div class="info-bar">{NAME}</div>'
  //  });
      infoWindowTemplate: function(atts) {
      //  return '<div class="iw-content"><h4>Median Age: ' + atts.NAME + '</h4></div>';
        var content = '<h4 style="color:white;background-color:#005789;padding:8px">'
                        + atts.NAME+'</h4>' ;
       //                  '<b>Name: </b>'+e.property["NAME"]+'<br>'
       //                  +'<b>Trail Status: </b>{STATUS}';

        return   $('#info-bar').html(content);
    },
});

       ags_light_rail.setMap(map);
       
  /*     ags_light_rail.addListener('click', function(e) {
               var content = //'<h4 style="color:white;background-color:#005789;padding:8px">'
                       // + e.feature.getProperty('NAME')+'</h4>'
                         '<b>Name: </b>{NAME}<br>'
                         +'<b>Trail Status: </b>{STATUS}';
